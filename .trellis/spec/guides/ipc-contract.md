...
| 概念 | Rust 侧 | 前端侧 | 例子 |
|---|---|---|---|
| 命令名 | snake_case 函数名 | `invoke("同名")`;封装函数名为其 camelCase | 示意:`get_settings` ↔ `getSettings()`;现存:`hide_launcher` ↔ `hideLauncher()`、`get_toggle_shortcut` ↔ `getToggleShortcut()`(`commands/launcher.rs` ↔ `src/lib/api.ts`) |
...
| 事件名 | `pub const X: &str = "domain://action"` | `EVENTS.X = "domain://action"` | `launcher://open`(`launcher.rs::LAUNCHER_OPENED` ↔ `EVENTS.LAUNCHER_OPENED`) |
