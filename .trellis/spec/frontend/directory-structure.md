...
│   ├── api.ts             # 唯一允许 import `@tauri-apps/api/core` 的 invoke 封装层:hideLauncher / getToggleShortcut
│   ├── events.ts          # Rust → 前端事件名常量 EVENTS + EventPayloads 类型表,与 launcher.rs 常量一一对应
│   ├── runtime.ts         # isTauriRuntime():是否运行在 Tauri WebView 内
│   ├── window.ts          # 唯一允许 import `@tauri-apps/api/window` 的文件;只做 setSize(隐藏走 api.ts 命令)
...
| 窗口 API 封装 | `src/lib/window.ts` | 动词短语,如 `resizeLauncherToContent()`;非 Tauri 环境 no-op;只放前端确实需要直接操作窗口的事(尺寸同步),显示 / 隐藏走 `api.ts` 命令 | 与 `lib/api.ts` 同一套「唯一入口 + 浏览器降级」规则(见 `ipc-guidelines.md` §1) |
